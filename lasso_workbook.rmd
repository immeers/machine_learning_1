...
data(duplicated(life[, c("Country", "Year")])) # Returns duplicated values
```
...
summary(life[,feat_vars]) #looking for NAs missing values
```
...
```{r}
# Fit lasso without selecting lambda, chooses its own lambda
...
plot_glmnet(fit_6, # Plot lasso coefficients by lambda
            xvar = "lambda") #shows how coefficients change for each variable depending value of (log) lambda
```
...
#Most variables have nas will need to impute, no missing values in hosue price

#Should visualise the outcome variable to see the distribution, check it is balanced
g_1 <- ggplot(house_price, aes(x = House.Price)) +
  geom_density(fill = "blue", alpha = 0.5) +
   theme_set(theme_bw(base_size = 22) ) +
  theme(panel.grid.major = element_blank(), # Remove grid
        panel.grid.minor = element_blank(), # Remove grid
        panel.border = element_blank(), # Remove grid
        panel.background = element_blank()) + # Remove grid 
  labs(x = "House Prices", title = "Distribution of Life Expectancy")
g_1

#Also can visualise for outliers

# Extract feature variables
feat_vars <- names(house_price)[names(house_price) != "House.Price"]

# Summarise feature variables

summary(house_price[,feat_vars]) #looking for NAs missing values

#visualise missing
vis_miss(house_price[, feat_vars])
...
house_price[,feat_vars] <- complete(imputed_values, 1) # Extract imputed data
summary(house_price[,feat_vars]) #summary after impute



#LINEAR
use_data <-  house_price[,2:12] #omit categorical variables (location)


fit_1 <- lm(House.Price ~., # Using ~. uses all other variables as explanatory variables
            data = use_data)
...
use_data2 <- use_data[names(use_data) != "House.Price"]

use_data2

# Scale explanatory variables
x_vars <- scale(use_data2)

# Create sequence of lambda values
lambda_seq <- seq(from = 0.1, to = 10, by = 0.1)

# Run lasso cross validation
fit_2 <- cv.glmnet(x = x_vars, # Set explanatory variables
                   y = use_data$House.Price, # Set response variable
                   alpha = 1, # Set alpha as 1 for lasso
                   lambda = lambda_seq, # Set lambda as sequence of lambda values
                   nfolds = 10) # Set number of folds as 10
...
#COMPARE LASSO AND LINEAR COEFS
temp <- coef(fit_1) # Extract coefficients from linear model
temp_2 <- as.vector(coef(fit_2)) # Extract coefficients from lasso model
cbind.data.frame(temp, temp_2) # Join coefficients together
