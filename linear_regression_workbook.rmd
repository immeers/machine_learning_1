...
tail(insurance) # View tail of dataset (Last rows), especially if human 
```
...
```{r}
# Create plot- use density if one variable or one numeric and one categoric
...
log_charges <- log(insurance$charges + 1) # Log transform insurance charges, adding 1 preserves 0s as 0
...
g_3 # Generate plot
#age is acting as a lower bound
```
...
g_5 # Generate plot if you can draw horizontal line on the shaded area then it is hard to find relationship
```
...
```{r Examine Model 2}
summary(fit_2)
#must interpret the intercept as relative to other, e,g. smokers relative to non smokers, nw se and sw relative to ne
#r makes its choice about what factor to use in intercept by alphabetical
```
